...
int main() {
	...
	/*load model*/
	std::shared_ptr<torch::jit::script::Module> module = torch::jit::load("D:\\data\\tanglin\\maskrcnn_c++\\model_960_gpu.pt");
	...
	cv::String txtfile1 = "D:\\data\\tanglin\\maskrcnn_c++\\result\\pose\\labels_pose\\";
	cv::String txtfile2 = "D:\\data\\tanglin\\maskrcnn_c++\\result\\pose\\result_pose\\";
	cv::String txtfile3 = "D:\\data\\tanglin\\maskrcnn_c++\\result\\pose\\scores_pose\\";
	/*read images*/
	std::vector<cv::String> filenames;
	//cv::String folder = "D:\\data\\tanglin\\maskrcnn_c++\\川大原图\\Picture2"; //images folder
	cv::String folder = "D:\\data\\tanglin\\maskrcnn_c++\\pict"; //images folder
																   
	cv::String save_folder = "D:\\data\\tanglin\\maskrcnn_c++\\result\\rpict\\"; //save folder
	...
	for (int i = 0; i<filenames.size(); ++i) {
		...
		string hz = ".txt";
		vector<string> txtname = split(filename_str[filename_str.size() - 1], '.');
		ofstream file1(txtfile1 + txtname[0] + hz, ios::out);
		ofstream file2(txtfile2 + txtname[0] + hz, ios::out);
		ofstream file3(txtfile3 + txtname[0] + hz, ios::out);
		...
		int num_out = 0;
		int num = 0;
		float box_in[1000][5]; // x1,y1,x2,y2,score,kind
		float result[500][5];
		...
		for (int p = 0;p < 4;p++)
		{
			...
			if (p == 0)
			{
				cv::resize(img1, img, img.size());  // must be bilinear interpolation,it fits our works more
			...
			{
				cv::resize(img2, img, img.size());  // must be bilinear interpolation,it fits our works more
				auto input_ = torch::tensor(at::ArrayRef<uint8_t>(img.data, img.rows * img.cols * 3)).view({ img.rows, img.cols, 3 });
				/* data -> model */
				std::vector<torch::jit::IValue> inputs;
				inputs.push_back(input_);
			...
			{
				cv::resize(img3, img, img.size());  // must be bilinear interpolation,it fits our works more
				auto input_ = torch::tensor(at::ArrayRef<uint8_t>(img.data, img.rows * img.cols * 3)).view({ img.rows, img.cols, 3 });
				/* data -> model */
				std::vector<torch::jit::IValue> inputs;
				inputs.push_back(input_);
			...
			{
				cv::resize(img4, img, img.size());  // must be bilinear interpolation,it fits our works more
				auto input_ = torch::tensor(at::ArrayRef<uint8_t>(img.data, img.rows * img.cols * 3)).view({ img.rows, img.cols, 3 });
				/* data -> model */
				std::vector<torch::jit::IValue> inputs;
				inputs.push_back(input_);
		...
		_nms(keep, &num_out, (const float*)box_in, num, 5, 0.5, 0);
		num = 0;
		for (int nn = 0;nn < num_out;++nn) {
			...
			file1 << result[num][4] << "\n";
			file2 << result[num][0] << " " << result[num][1] << " " << result[num][2] << " " << result[num][3] << "\n";
			file3 << bsort[keep[nn]].score << "\n";
			++num;
		}
		file1.close();
		file2.close();
		...
		string hh = ".jpg";
		cv::imwrite(save_folder + txtname[0] + hh, img_);
		clock_t end = clock();
