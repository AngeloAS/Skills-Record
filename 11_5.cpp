...
int main() {
	...
	// assign a fixed scale
	int x_in = 1312, y_in = 960; 

	/* load opencv dill */
	void* custom_op_lib = dlopen("custom_ops_no_opencv.cp36-win_amd64.dll", RTLD_NOW | RTLD_GLOBAL);
	...
	module->to(device);
	std::cout << "finish load model" << endl;

	/* read images */
	std::vector<cv::String> filenames;
	cv::String folder = "D:\\data\\tanglin\\maskrcnn_c++\\pict";   //images folder													   
	cv::String save_folder = "D:\\data\\tanglin\\maskrcnn_c++\\result\\rrpict\\";   //save folder
	...
	for (int i = 0; i<filenames.size(); ++i) {
		...
		int num_out = 0;
		int num = 0;
		float box_in[1000][5]; // x1,y1,x2,y2,score
		float result[500][5];  // x1,y1,x2,y2,score
		...
		for (int p = 0;p < 4;p++)
		{
			...
			if (p == 0)
			{
				...
				std::vector<c10::IValue, std::allocator<c10::IValue>> res;
		...
		num = 0;
		// sort bbox by score
		// std::sort(bsort.begin(), bsort.end(), sort_Bbox);
		...
		// draw bbox On the original picture
		...
		std::cout << "peoplecount = " << standNum + sitNum + lieNum << "\nstanding = " << standNum << "\nsitting = " << sitNum << "\nlying = " << lieNum << endl;
		cv::imwrite(save_folder + filename_str[filename_str.size()-1], img_);
		clock_t end = clock();
		std::cout << (end -start) / 1000.0 << endl;
	}
